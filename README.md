...
<head>
    ...
    <title>SUPER CLEAN ERP v30.0 - VERSION INDUSTRIELLE FINALE</title>
    ...
    <style>
        ...
        .logo-main { width: 70px; height: 70px; border-radius: 50%; border: 3px solid var(--primary); object-fit: cover; }
        .brand-title { font-weight: 900; color: var(--primary); font-size: 22px; margin: 8px 0 2px 0; letter-spacing: -1px; }
        .slogan-text { font-size: 12px; color: #555; font-style: italic; font-weight: 600; letter-spacing: 0.5px; }

        /* --- NAVIGATION BASSE DYNAMIQUE --- */
        ...
        h3 { margin-top: 0; font-size: 16px; border-bottom: 2px solid var(--light); padding-bottom: 10px; color: var(--primary); }
        ...
        .btn-main { background: var(--primary); color: white; border: none; padding: 16px; border-radius: 15px; font-weight: 800; width: 100%; cursor: pointer; text-transform: uppercase; letter-spacing: 1px; }
        .btn-wa { background: #25D366; color: white; border: none; padding: 12px; border-radius: 12px; font-size: 11px; font-weight: 700; width: 100%; margin: 5px 0; cursor: pointer; display: flex; align-items: center; justify-content: center; }

        /* --- TRAÇABILITÉ ET BADGES --- */
        .log-row { font-size: 10px; padding: 8px; border-bottom: 1px solid #eee; font-family: 'Courier New', monospace; display: flex; justify-content: space-between; }
        .solde-badge { background: var(--danger); color: white; padding: 5px 10px; border-radius: 8px; font-size: 11px; font-weight: 900; display: inline-block; margin-bottom: 10px; }
        ...
        .stock-tag { padding: 4px 8px; border-radius: 6px; font-size: 11px; font-weight: 800; }
        .low-stock { background: #ffebeb; color: var(--danger); border: 1px solid var(--danger); animation: blink 1s infinite; }
    ...
    <div id="main-content" style="padding-bottom: 120px;">
        ...
        <div id="section-caisse" class="tab-content">
            <div class="card">
                ...
                <div style="background:#f8f9fa; padding:15px; border-radius:15px; border:1px dashed #ddd; margin-top:10px;">
                    <label style="font-size:11px; font-weight:800; color:var(--primary);">CHOISIR PRESTATION :</label>
                    <select id="select-article" onchange="majPrixSuggere()"></select>
                    <label style="font-size:11px; font-weight:800; color:var(--primary);">PRIX UNITAIRE AJUSTÉ (F CFA) :</label>
            ...
            <div style="padding: 0 15px;">
                <button onclick="validerCommande()" class="btn-main" style="height:70px;">VALIDER & IMPRIMER TICKET</button>
            </div>

            <div class="card" style="border: 2px solid var(--danger);">
                <h3>💸 SORTIES DE CAISSE (ACHATS)</h3>
        ...
        <div id="section-stocks" class="tab-content hidden">
            <div class="card">
                ...
                <div id="admin-stock-ui" class="hidden">
                    ...
                    <h4>📦 AJUSTEMENT MANUEL (INVENTAIRE)</h4>
        ...
        <div id="section-admin" class="tab-content hidden">
            <div class="card" style="background:var(--primary); color:white; text-align:center;">
                <small>NET EN CAISSE AUJOURD'HUI</small>
            ...
            <div class="card">
                ...
                <input type="text" id="ns-log" placeholder="Nom d'utilisateur (Login)">
...
/* ==========================================
   ...
   ========================================== */
let db = JSON.parse(localStorage.getItem('sc_db_v30')) || [];
let staff = JSON.parse(localStorage.getItem('sc_staff_v30')) || [
    {nom: "Administrateur", login: "admin", pass: "0000", role: "ADMIN", actif: true}
];
let stocks = JSON.parse(localStorage.getItem('sc_stocks_v30')) || [
...
let logs = JSON.parse(localStorage.getItem('sc_logs_v30')) || [];
let achats = JSON.parse(localStorage.getItem('sc_achats_v30')) || [];
let panier = [], currentUser = null;

/* ==========================================
   3. AUTHENTIFICATION ET SÉCURITÉ ROLES
   ========================================== */
function login() {
    ...
    if(u) {
        ...
        filtrerMenu();
        initDropdownPrestations();
    } else alert("Identifiants incorrects !");
}

function filtrerMenu() {
    const r = currentUser.role;
    const hasCaisse = (r === "ADMIN" || r === "Caissière" || r === "Assistant");
...
function addLog(action) {
    ...
    localStorage.setItem('sc_logs_v30', JSON.stringify(logs));
}

function initDropdownPrestations() {
    const s = document.getElementById('select-article');
    s.innerHTML = '<option value="">-- Choisir un vêtement --</option>';
...
function validerCommande() {
    ...
    if(!nom || !dr || panier.length === 0) return alert("Champs obligatoires manquants !");
    
    // Déduction auto stocks : 50ml savon, 10ml parfum, 1 housse par vêtement
    stocks[0].qte -= (panier.length * 0.05);
    stocks[1].qte -= (panier.length * 0.01);
    stocks[2].qte -= panier.length;
    localStorage.setItem('sc_stocks_v30', JSON.stringify(stocks));
    ...
    db.unshift(cmd); localStorage.setItem('sc_db_v30', JSON.stringify(db));
...
function imprimerTicket(c) {
    document.getElementById('tk-body').innerHTML = `
        ...
        ${c.articles.map(a => `${a.nom}: ${a.prix}F`).join('<br>')}<hr>
        TOTAL: ${c.total} F<br>AVANCE: ${c.paye} F (${c.mode})<br><b>SOLDE: ${c.total - c.paye} F</b><hr>
        RDV RETRAIT: ${c.retrait}
    `;
    window.print();
}
...
function enregistrerAchat() {
    ...
    if(motif && mont) {
        ...
        localStorage.setItem('sc_achats_v30', JSON.stringify(achats));
        addLog(`ACHAT CAISSE : ${motif} (-${mont}F)`);
...
function renderAtelier() {
    ...
    document.getElementById('atelier-liste').innerHTML = db.filter(c => c.statut !== "Livré").map(c => `
        <div class="card ${c.retrait < today ? 'retard-card' : ''}">
            ...
            ${isCaisse ? `<button class="btn-wa" onclick="envoyerWA('${c.tel}', '${c.client}', '${c.id}')">📲 WHATSAPP : DISPONIBLE</button>` : ''}
...
function finirArt(cid, auid) {
    ...
    addLog(`Vêtement prêt : ${a.nom} (Ticket #${cid})`);
    localStorage.setItem('sc_db_v30', JSON.stringify(db)); renderAtelier();
}
...
function envoyerWA(tel, client, id) {
    const msg = `Bonjour ${client}, votre commande SUPER CLEAN #${id} est prête. Merci de passer la récupérer.`;
    window.open(`https://wa.me/237${tel}?text=${encodeURIComponent(msg)}`);
}

/* ==========================================
   6. ADMIN, STOCKS & RAPPORT
   ========================================== */
function envoyerRapportJournalier() {
    const today = new Date().toDateString();
    const tksToday = db.filter(t => new Date(t.timestamp).toDateString() === today);
    const achToday = achats.filter(a => new Date(a.timestamp).toDateString() === today);
    
    let cash=0, om=0, momo=0;
    tksToday.forEach(t => { if(t.mode==="Cash") cash+=t.paye; if(t.mode==="OM") om+=t.paye; if(t.mode==="Momo") momo+=t.paye; });
    let totalAchats = achToday.reduce((a,b)=>a+b.mont, 0);
    
    const msg = `📊 *RAPPORT SUPER CLEAN*\n📅 ${new Date().toLocaleDateString()}\n👤 Agent : ${currentUser.nom}\n------------------\n🎟️ Tickets : ${tksToday.length}\n💵 Cash : ${cash}F\n🍊 Orange : ${om}F\n🟡 MoMo : ${momo}F\n❌ Achats : -${totalAchats}F\n------------------\n💰 *NET EN CAISSE : ${cash+om+momo-totalAchats}F*`;
...
function majStockManuel() {
    ...
    addLog(`INVENTAIRE MANUEL : ${s.nom} (${q > 0 ? '+':''}${q})`);
    localStorage.setItem('sc_stocks_v30', JSON.stringify(stocks)); renderStocks();
}
...
function renderAdmin() {
    ...
    document.getElementById('staff-list').innerHTML = staff.map((s,i) => `
        <div style="display:flex; justify-content:space-between; padding:10px; border-bottom:1px solid #eee;">
            ...
            ${i!==0 ? `<button onclick="toggleAccess(${i})" style="font-size:10px; padding:5px; background:var(--dark); color:white; border:none; border-radius:5px;">BASCULER</button>` : ''}
        </div>`).join('');
}

function toggleAccess(i) { staff[i].actif = !staff[i].actif; localStorage.setItem('sc_staff_v30', JSON.stringify(staff)); renderAdmin(); }
...
function ajouterEmploye() {
    ...
    if(n&&l&&p) { staff.push({nom:n, login:l, pass:p, role:r, actif:true}); localStorage.setItem('sc_staff_v30', JSON.stringify(staff)); addLog(`Création employé: ${n}`); renderAdmin(); }
}
...
function exportData() {
    ...
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download=`SUPER_CLEAN_FULL_BACKUP.json`; a.click();
}
...
function importData() {
    ...
    reader.onload = (e) => {
        ...
        localStorage.setItem('sc_db_v30', JSON.stringify(d.db));
        localStorage.setItem('sc_staff_v30', JSON.stringify(d.staff));
        localStorage.setItem('sc_stocks_v30', JSON.stringify(d.stocks));
        localStorage.setItem('sc_logs_v30', JSON.stringify(d.logs));
        location.reload();
    };
