...
<head>
    ...
    <title>SUPER CLEAN PRESSING ERP v20.0 - SYSTÈME FINAL</title>
    ...
    <style>
        ...
        /* --- BRANDING & HEADER --- */
        .header { background: white; padding: 15px; text-align: center; border-bottom: 3px solid var(--primary); sticky: top; z-index: 1000; }
        ...
        .nav-bar button { background: #f8f9fa; border: none; color: #7f8c8d; font-weight: 800; font-size: 8px; padding: 12px 2px; border-radius: 14px; cursor: pointer; transition: 0.2s; }
        ...
        .btn-wa { background: #25D366; color: white; border: none; padding: 10px; border-radius: 10px; font-size: 11px; font-weight: 700; width: 100%; margin: 4px 0; cursor: pointer; display: flex; align-items: center; justify-content: center; }
        .btn-danger { background: var(--danger) !important; }

        /* --- ZONE TECHNIQUE --- */
        .retard-card { border: 2px solid var(--danger) !important; background: #fff5f5 !important; animation: blink-border 2s infinite; }
        ...
        /* --- LOGIN --- */
        #auth-screen { height: 100vh; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, var(--primary), #00b4d8); }
        .login-box { background: white; padding: 40px; border-radius: 30px; width: 85%; max-width: 350px; text-align: center; box-shadow: 0 20px 40px rgba(0,0,0,0.2); }
...
<div id="auth-screen">
    <div class="login-box">
        ...
        <h2 style="margin:0; color:var(--dark);">SUPER CLEAN PRESSING</h2>
        <p style="font-size:11px; color:var(--primary); font-weight:700; margin-bottom:25px; text-transform:uppercase;">L'excellence au service de votre image</p>
        <input type="text" id="u-login" placeholder="Identifiant">
        <input type="password" id="u-pass" placeholder="Mot de passe">
        <button onclick="login()" class="btn-main">ENTRER DANS LE SYSTÈME</button>
    </div>
</div>
...
<div id="app" class="hidden">
    <div class="header">
        ...
        <div id="user-info" style="font-size:10px; margin-top:5px; font-weight:700; color:#666;"></div>
    </div>

    <div id="main-content" style="padding-bottom: 100px;">
        
        <div id="section-caisse" class="tab-content">
            <div class="card">
                <h3 style="margin-top:0;">📥 Réception Client</h3>
                ...
                <div style="background:#f8f9fa; padding:15px; border-radius:15px; border:1px dashed #ddd;">
                    ...
                    <button onclick="ajouterAuPanier()" class="btn-main" style="background:var(--dark); margin:0;">+ AJOUTER L'ARTICLE</button>
                </div>
            </div>
            ...
            <div style="padding: 0 15px;">
                ...
                <button onclick="envoyerRapportJournalier()" class="btn-wa" style="background:#075E54; padding:15px;">📊 RAPPORT JOURNALIER (WA)</button>
            </div>
        </div>
        ...
        <div id="section-cloud" class="tab-content hidden">
            <div class="card" style="text-align:center;">
                ...
                <p style="font-size:12px; color:#666;">Exportez votre base de données pour la sécuriser en dehors du téléphone.</p>
        ...
        <div id="section-admin" class="tab-content hidden">
            ...
            <div class="card">
                ...
                <button onclick="ajouterEmploye()" class="btn-main" style="background:var(--success);">CRÉER COMPTE</button>
            </div>
        </div>
    </div>
...
<div id="ticket-print" style="display:none; text-align:center; padding:10px;">
    ...
    --------------------------------<br>
    <div id="tk-body" style="text-align:left; font-family:monospace; font-size:12px;"></div>
    --------------------------------<br>
    "Le bien être de vos vêtements"
</div>
...
const GRILLE_OFFICIELLE = {
    ...
    "ENSEMBLES": {"Costume simple":2000,"Costume luxe":2500,"Boubou femme":1500,"Boubou homme 2P":1500,"Boubou homme 3P":2500,"Pyjama 2P":1000,"Jogging":1500},
...
let db = JSON.parse(localStorage.getItem('sc_db_v20')) || [];
let staff = JSON.parse(localStorage.getItem('sc_staff_v20')) || [{nom:"Directeur", login:"admin", pass:"0000", role:"ADMIN"}];
...
function login() {
    ...
    if(u) {
        ...
        document.getElementById('user-info').innerText = `${u.nom} (${u.role})`;
        filtrageRoles();
        initApp();
    } else alert("Identifiants incorrects");
}

function filtrageRoles() {
    const r = currentUser.role;
    document.getElementById('btn-caisse').classList.toggle('hidden', (r !== "ADMIN" && r !== "Caissière"));
    ...
    tab((r === "ADMIN" || r === "Caissière") ? 'section-caisse' : 'section-ZONE TECHNIQUE');
}
...
function validerCommande() {
    ...
    const cmd = { id: Math.floor(1000+Math.random()*8999), client: nom, tel: tel, articles: [...panier], total: panier.reduce((a,b)=>a+b.prix,0), paye: parseInt(document.getElementById('paye-caisse').value)||0, mode: document.getElementById('mode-pay').value, retrait: dr, statut: "Atelier", timestamp: new Date().getTime(), agent: currentUser.nom };
    db.unshift(cmd); localStorage.setItem('sc_db_v20', JSON.stringify(db));
...
function renderAtelier() {
    ...
    document.getElementById('atelier-liste').innerHTML = db.filter(c => c.statut !== "Livré").map(c => `
        <div class="card ${c.retrait < today ? 'retard-card' : ''}">
            ...
            <div class="solde-badge">${c.total-c.paye > 0 ? 'À PAYER : '+(c.total-c.paye)+' F' : 'SOLDE PAYÉ'}</div><br>
            ${(currentUser.role === "ADMIN" || currentUser.role === "Caissière") ? `<button class="btn-wa" onclick="window.open('https://wa.me/237${c.tel}?text=Prêt')">📲 WHATSAPP PRÊT</button>` : ''}
            <button onclick="livrer(${c.id})" class="btn-main" style="margin-top:5px; background:var(--dark);">LIVRER ET FERMER</button>
        </div>`).join('');
}
...
function livrer(id) {
    if(confirm("Encaisser le solde et livrer ?")) {
        ...
        localStorage.setItem('sc_db_v20', JSON.stringify(db)); renderAtelier();
    }
}
...
function envoyerRapportJournalier() {
    ...
    const cmdToday = db.filter(t => new Date(t.timestamp).toDateString() === today);
    let cash=0, om=0, momo=0;
    cmdToday.forEach(t => { if(t.mode==="Cash") cash+=t.paye; if(t.mode==="OM") om+=t.paye; if(t.mode==="Momo") momo+=t.paye; });
...
function importData() {
    ...
    reader.onload = (e) => {
        ...
        localStorage.setItem('sc_db_v20', JSON.stringify(data.db));
        localStorage.setItem('sc_staff_v20', JSON.stringify(data.staff));
        location.reload();
    };
...
function ajouterEmploye() {
    ...
    if(n&&l&&p) { staff.push({nom:n, login:l, pass:p, role:r}); localStorage.setItem('sc_staff_v20', JSON.stringify(staff)); tab('section-admin'); }
}

function rechercherClient() {
    const t = document.getElementById('tel-client').value;
    if(t.length>=8) { const f = db.find(x => x.tel.includes(t)); if(f) document.getElementById('client-nom').value = f.client; }
}
</script>
</body>
</html>
