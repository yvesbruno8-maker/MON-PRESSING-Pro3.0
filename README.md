...
<head>
    ...
    <title>SUPER CLEAN ERP v40.0 - VERSION INTÉGRALE</title>
    ...
    <style>
        ...
        /* --- STYLES SPÉCIFIQUES --- */
        .item-row { display: flex; justify-content: space-between; align-items: center; padding: 15px; background: #f8f9fa; border-radius: 15px; margin-bottom: 8px; border-left: 6px solid var(--primary); font-size: 13px; }
        ...
        .status-pill { padding: 4px 10px; border-radius: 20px; font-size: 10px; font-weight: 900; text-transform: uppercase; }
        .pill-active { background: #dcfce7; color: #166534; }
        .pill-blocked { background: #fee2e2; color: #991b1b; }
        .retard-card { border: 2px solid var(--danger) !important; background: #fff5f5 !important; animation: blink 2s infinite; }
...
<div id="auth-screen">
    <div style="height: 100vh; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, var(--primary) 0%, #00b4d8 100%);">
        <div class="card" style="width: 90%; max-width: 400px; text-align: center; padding: 50px;">
            ...
            <button onclick="login()" class="btn-main">CONNEXION AU SYSTÈME</button>
        </div>
    </div>
</div>
...
<div id="app" class="hidden">
    ...
    <div id="main-content" style="padding-bottom: 130px;">
        ...
        <div id="section-caisse" class="tab-content">
            <div class="card">
                <h3>📥 RÉCEPTION & ENREGISTREMENT</h3>
                ...
                <label style="font-size:11px; font-weight:800; color:#777; margin-left:5px;">DATE DE RETRAIT ESTIMÉE :</label>
                ...
                <div style="background:#f8f9fa; padding:20px; border-radius:18px; border:1px dashed #ddd; margin-top:10px;">
                    <label style="font-size:11px; font-weight:800; color:var(--primary);">DÉSIGNATION DE L'ARTICLE :</label>
                    <select id="select-article" onchange="majPrixInitial()"></select>
                    <label style="font-size:11px; font-weight:800; color:var(--primary);">PRIX APPLIQUÉ (F CFA) :</label>
            ...
            <div class="card" style="background:var(--dark); color:white;">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                    <span style="opacity:0.8; font-size: 14px;">TOTAL À ENCAISSER :</span> 
                ...
                <label style="font-size:11px; color:#aaa; font-weight:700;">AVANCE VERSÉE AU DÉPÔT :</label>
            ...
            <div class="card" style="border: 2px solid var(--danger);">
                <h3>💸 SORTIE DE CAISSE (ACHATS)</h3>
                <input type="text" id="achat-motif" placeholder="Motif de la dépense (ex: Charbon, Savon...)">
        ...
        <div id="section-rapport" class="tab-content hidden">
            <div class="card">
                <h3>📊 ESPACE RAPPORT JOURNALIER</h3>
                <div id="rapport-details" style="margin-bottom: 25px;"></div>
                <button onclick="envoyerRapportWA()" class="btn-wa">📲 ENVOYER LE BILAN PAR WHATSAPP</button>
            </div>
        </div>
        ...
        <div id="section-performance" class="tab-content hidden">
            <div class="card">
                ...
                <div class="item-row"><span>Mensuel (Mois)</span> <b id="p-month">0 F</b></div>
        ...
        <div id="section-logs" class="tab-content hidden">
            <div class="card">
                <h3>📜 JOURNAL D'ACTIVITÉS (SUIVI TEMPS RÉEL)</h3>
                ...
                <button onclick="clearLogs()" class="btn-main" style="background:var(--danger); margin-top:20px; font-size:11px; padding:10px;">EFFACER LE JOURNAL</button>
            </div>
        </div>
        ...
        <div id="section-admin" class="tab-content hidden">
            <div class="card" style="border: 3px solid var(--primary);">
                ...
                <select id="ns-role">
                    ...
                    <option value="Zone Technique">Zone Technique (Laveur/Repasseur)</option>
...
// ==========================================
let GRILLE = JSON.parse(localStorage.getItem('sc_grille_v40')) || {
    "LES HAUTS": {
    ...
    "LINGES DE LIT": {
        "1 Drap": 750, "Ensemble 1 drap+taie": 1500, "Ensemble 2 drap+taies": 2000, 
        "Couvre lit / Couette": 2000, "Housse de couette": 2500, "Couverture": 4000
    },
    "VÊTEMENTS ENFANTS": {
...
// ==========================================
let db = JSON.parse(localStorage.getItem('sc_db_v40')) || [];
let staff = JSON.parse(localStorage.getItem('sc_staff_v40')) || [
    {nom: "Administrateur", login: "admin", pass: "0000", role: "ADMIN", actif: true}
];
let stocks = JSON.parse(localStorage.getItem('sc_stocks_v40')) || [
...
let logs = JSON.parse(localStorage.getItem('sc_logs_v40')) || [];
let achats = JSON.parse(localStorage.getItem('sc_achats_v40')) || [];
...
function login() {
    ...
    if(u) {
        if(!u.actif && u.role !== "ADMIN") return alert("🚫 ACCÈS BLOQUÉ : Veuillez contacter l'administrateur.");
        currentUser = u;
...
function filtrerRoles() {
    ...
    // Verrouillage Physique de la Navigation
...
function addLog(action) {
    ...
    localStorage.setItem('sc_logs_v40', JSON.stringify(logs.slice(0, 1000)));
}
...
function validerCommande() {
    ...
    if(!nom || !dr || panier.length === 0) return alert("Veuillez remplir les informations client.");
    
    // Déduction auto stocks (0.05 savon / 1 housse par article)
    stocks[0].qte -= (panier.length * 0.05);
    stocks[2].qte -= panier.length;
    localStorage.setItem('sc_stocks_v40', JSON.stringify(stocks));
    ...
    db.unshift(cmd); localStorage.setItem('sc_db_v40', JSON.stringify(db));
    addLog(`Création Ticket #${cmd.id} (${cmd.total}F)`);
    imprimerTicket(cmd);
    location.reload();
}
...
function enregistrerAchat() {
    ...
    if(motif && mont) {
        ...
        localStorage.setItem('sc_achats_v40', JSON.stringify(achats));
        addLog(`SORTIE DE CAISSE : ${motif} (-${mont}F)`);
        alert("Achat enregistré."); location.reload();
    }
}
...
function renderRapport() {
    ...
    document.getElementById('rapport-details').innerHTML = `
        <div class="item-row"><span>Nbre Tickets du jour :</span> <b>${tks.length}</b></div>
        ...
        <div class="item-row" style="color:var(--danger)"><span>Sorties (Achats) :</span> <b>-${totalAchats} F</b></div>
...
function renderAtelier() {
    ...
    document.getElementById('atelier-liste').innerHTML = db.filter(c => c.statut !== "Livré").map(c => `
        <div class="card ${c.retrait < today ? 'retard-card' : ''}">
            ...
            <div class="solde-badge">SOLDE DU : ${c.total-c.paye} F</div><br>
            ${c.articles.map(a => `
                <div class="item-row">
                    <span>${a.nom}</span>
            ...
            <button onclick="window.open('https://wa.me/237${c.tel}?text=Bonjour%20votre%20vêtement%20est%20prêt')" style="background:#25D366; color:white; border:none; padding:10px; border-radius:10px; font-size:10px; margin-bottom:10px;">APPEL WHATSAPP CLIENT</button>
            <button onclick="livrer(${c.id})" class="btn-main" style="background:var(--dark);">LIVRER ET SOLDE</button>
        </div>`).join('');
}
...
function finirArt(cid, auid) {
    ...
    addLog(`Article prêt : ${a.nom} (#${cid})`);
    localStorage.setItem('sc_db_v40', JSON.stringify(db)); renderAtelier();
}

function livrer(id) {
    if(confirm("Confirmer la livraison totale ?")) {
...
// ==========================================
// 7. PERFORMANCE & ANALYTICS (ADMIN SEUL)
// ==========================================
...
// ==========================================
// 8. ADMIN SOUVERAIN (GÉRANCE & GRILLE)
// ==========================================
function renderAdmin() {
    document.getElementById('staff-list').innerHTML = staff.map((s,i) => `
        <div style="display:flex; justify-content:space-between; padding:15px; border-bottom:1px solid #eee;">
            ...
            ${i!==0 ? `<button onclick="toggleAccess(${i})" style="font-size:10px; padding:10px; border-radius:8px; background:var(--dark); color:white; border:none;">BASCULER ACCÈS</button>` : ''}
        </div>`).join('');
}

function toggleAccess(i) { staff[i].actif = !staff[i].actif; localStorage.setItem('sc_staff_v40', JSON.stringify(staff)); renderAdmin(); addLog(`Accès modifié pour ${staff[i].nom}`); }
...
function ajouterEmploye() {
    ...
    if(n&&l&&p) { staff.push({nom:n, login:l, pass:p, role:r, actif:true}); localStorage.setItem('sc_staff_v40', JSON.stringify(staff)); addLog(`Création compte : ${n}`); renderAdmin(); }
}
...
function ajouterNouveauProduit() {
    ...
    if(cat && nom && prix) {
        ...
        GRILLE[cat][nom] = prix;
        localStorage.setItem('sc_grille_v40', JSON.stringify(GRILLE));
        alert("GRILLE MISE À JOUR !"); initDropdownArticles();
    }
}

// ==========================================
// 9. NAVIGATION & SYSTEME
// ==========================================
...
function majStockManuel() {
    ...
    stocks.find(s => s.id === id).qte += q; localStorage.setItem('sc_stocks_v40', JSON.stringify(stocks)); renderStocks();
}
...
function clearLogs() { if(confirm("Effacer ?")) { logs=[]; localStorage.setItem('sc_logs_v40', "[]"); tab('section-logs'); } }

function exportData() {
    const b = new Blob([JSON.stringify({db, staff, stocks, achats, logs, GRILLE})], {type:"application/json"});
    const a = document.createElement('a'); a.href = URL.createObjectURL(b); a.download=`SUPER_CLEAN_FULL_BACKUP.json`; a.click();
}

function importData() {
    const reader = new FileReader(); reader.onload = (e) => {
        const d = JSON.parse(e.target.result);
        localStorage.setItem('sc_db_v40', JSON.stringify(d.db));
        localStorage.setItem('sc_staff_v40', JSON.stringify(d.staff));
        localStorage.setItem('sc_stocks_v40', JSON.stringify(d.stocks));
        localStorage.setItem('sc_grille_v40', JSON.stringify(d.GRILLE));
        location.reload();
